...
static void office_init(office_t *o, unsigned seats) {
  ...
  pthread_cond_init(&o->queue, NULL);
  pthread_cond_init(&o->clerk, NULL);
  pthread_cond_init(&o->processed, NULL);
...
static void office_destroy(office_t *o) {
  ...
  pthread_cond_destroy(&o->queue);
  pthread_cond_destroy(&o->clerk);
  pthread_cond_destroy(&o->processed);
}

static bool customer_walk_in(office_t *o) {
  /* TODO: No seats then leave, otherwise wait for a clerk call. */
  pthread_mutex_lock(&o->mutex);
  if (o->waiting == o->seats) {
  ...
  pthread_cond_signal(&o->clerk);
  pthread_cond_wait(&o->queue, &o->mutex);

  pthread_cond_wait(&o->processed, &o->mutex);
...
static void clerk_wait(office_t *o) {
  ...
  pthread_mutex_lock(&o->mutex);
  if (o->waiting > 0)
    pthread_cond_signal(&o->queue);
  else
    pthread_cond_wait(&o->clerk, &o->mutex);
  pthread_mutex_unlock(&o->mutex);
}
...
static void clerk_done(office_t *o) {
  ...
  pthread_mutex_lock(&o->mutex);
  pthread_cond_signal(&o->processed);
  pthread_mutex_unlock(&o->mutex);
}
