...
static void sigsegv_handler(int signum, siginfo_t *info, void *data) {
  ...
  void *page_addr = (void *)((uintptr_t)fault_addr & ~((uintptr_t)pagesize - 1));
  ...
  if (fault_addr < ADDR_START || fault_addr >= ADDR_END) {
    safe_printf("Fault at rip=%p accessing %p! Address not mapped - terminating!\n", rip, fault_addr);
    exit(128+SIGSEGV);
  }

  if (info->si_code == SEGV_MAPERR) {
    safe_printf("Fault at rip=%p accessing %p! Map missing page at %p!\n", rip, fault_addr, page_addr);
    mmap_page(page_addr, PROT_READ | PROT_WRITE);
  }
  else if (info->si_code == SEGV_ACCERR) {
    safe_printf("Fault at rip=%p accessing %p! Make page at %p writable.\n", rip, fault_addr, page_addr);
    mmap_page(page_addr, PROT_READ | PROT_WRITE);
  }
