...
*/
  ...
  db->entry = (entry_t *) Mmap(NULL, size * sizeof(entry_t), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
  Madvise(db->entry, size * sizeof(entry_t), MADV_RANDOM);
  db->size = size;
  // the name argument is allocated on stack, so 
  db->name = strdup(name);
  Close(fd);
}
...
static void doit(const char *path, op_t mode) {
  ...
  if (isatty(STDIN_FILENO)) {
    ...
    struct stat stat;
    fstat(fileno(stdin), &stat);
    char *buf = Mmap(NULL, stat.st_size, PROT_READ | PROT_WRITE, MAP_PRIVATE, fileno(stdin), 0);
    char *line = buf;
    while((line = consume_line(line, &db, mode)) != NULL);
