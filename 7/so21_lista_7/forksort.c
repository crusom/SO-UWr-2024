...
static int QuickSort(long table[], size_t left, size_t right) {
  ...
  if (right - left > FORKSORT_MIN) {
    do_fork = true;
    pid_t pid;
    // dziecko oddelegowane, rodzic wraca
    if ((pid = fork()) != 0)
      return pid;
  }
  ...
  if (left < right) {
    if (right - left <= INSERTSORT_MAX) {
      ...
      if (pid_right != -1)
        Waitpid(pid, NULL, 0);
      // jestesmy oddelegowanym dzieckiem, wiec wychodzimy
      if (do_fork)
        exit(0);
    }
  }
...
int main(void) {
  ...
  /* TODO: Allocate table... */
  long *table = (long*) Mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_ANON | MAP_SHARED, -1, 0);
