...
static noreturn void coro_switch(int v) {
  ...
  // jeśli coroutine wrócił, to znaczy, że przetworzył cały tekst, więc może już wyjść z kolejki
  dprintf(2,"coro_Switch %d\n",v);
  if (v == EOF) {
  ...
  if (!running) running = TAILQ_FIRST(&runqueue);
...
static int coro_yield(int v) {
  ...
  if (nv == 0) {
    dprintf(2,"coro_yield set\n");
    coro_switch(v);
  }
  dprintf(2,"coro_yield jmp\n");
  return nv;
}
...
static void func_2(int ch) {
  ...
  //fprintf(stderr, "func_2: removed = %d\n", removed);
  dprintf(2, "func_2: removed = %d\n", removed);
}
...
static void func_3(int ch) {
  ...
  //fprintf(stderr, "func_3: printed = %d\n", printed);
  dprintf(2, "func_3: printed = %d\n", printed);
}
